/*
  Example, how to read data from a BME680 (here: GY-BME280 5V/3.3V) and display the values on a HELTEC WIFI Kit 32 display
...
#define PIN_SDA 4                   //Pins for onboard display
#define PIN_SCL 15
...
void loop()
{
  ...
  String tempvar = String(adjusted_temp) + (" °C");
  display.drawString( 0, 0, tempvar);

  tempvar = String(adjusted_humi) + (" %");
  ...
  // print on display
  display.drawString( 0, 40, calc_IAQ(temp_score + hum_score + gas_score));
